...
1. **FBX (`.fbx`) 우선 지원**
2. `.obj` 는 선택적 2차 지원
3. `glTF 2.0 (`.gltf`, `.glb`)` 는 추후 재검토
4. 그 외 포맷은 후순위

핵심 이유:

- 현재 구현은 FBX 정적 메시 프리뷰와 텍스처 연결 경로를 기준으로 구성되어 있다.
- DCC 툴에서 바로 내보낸 FBX 테스트 자산을 우선 검증하기 쉽다.
...
- `.fbx` 는 현재 구현된 모델 프리뷰 경로와 직접 맞물린다.
- `.obj` 는 단순 메시 테스트에는 좋지만 머티리얼/확장성 면에서 제한적이다.
- `.gltf`, `.glb` 는 추후 별도 로더 전략과 런타임 정책 정리 후 재도입하는 편이 안전하다.
...
> **WebGL2 기반으로 FBX 모델을 올리고, 사용자가 작성한 GLSL 셰이더의 uniform을 자동으로 UI에 노출하여 값과 텍스처를 연결해 볼 수 있는 셰이더 플레이그라운드**
